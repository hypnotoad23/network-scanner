...
Из-за ограничений сетевого стека Windows и особенностей драйвера Npcap, сканер может работать нестабильно (обнаруживать не все устройства или только хост-машину). 
**Для корректной работы рекомендуется:**
* Запускать сканер с флагом `-i` и указанием полного имени адаптера.
* Отключать VPN и виртуальные сетевые адаптеры (Docker, WSL, VirtualBox).
* Использовать Linux или Docker-контейнер для 100% точности сканирования.
